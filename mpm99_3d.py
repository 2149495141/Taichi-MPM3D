...
#dim, n_grid, steps, dt, res = 3, 64, int(2e-3 // 5e-4), 5e-4, 500  # 更友好的配置
...
def initialize():
    for i in range(n_particles):  # 初始化粒子的位置
        x[i] = [ti.random() * 0.1 + 0.3 + 0.08 * (i // group_size),  # 材质块x长度(随机*x偏移量） + 所有粒子的在x轴位置 + 材质块x轴的相对间隔(x偏移量*(粒子i//材质块粒子数，粒子i超过材质块粒子数时说明在下一材质块内，偏移一个整数量))
                ti.random() * 0.2 + 0.05 + 0.32 * (i // group_size),  # 材质块y长度(随机*y偏移量） + 所有粒子的在y轴位置 + 材质块y轴的相对间隔(y偏移量*(例如:材质块粒子数=21845, 21845//21845=1, 粒子在第二个材质块偏移量为1))
